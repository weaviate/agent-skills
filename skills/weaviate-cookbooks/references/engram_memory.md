...
Two things the examples don't show: `search` returns a `SearchResults` object (a `Sequence` of memories — iteration, `len()`, and indexing work), not a plain `list`, so annotate helpers accordingly. And results are filled up to `limit` even when weakly relevant: `score` is higher-is-better, but its range depends on retrieval type — hybrid is normalised roughly 0–1, while vector and BM25 scores run far lower. Apply a relevance cutoff before injecting memories into a prompt; ~0.5 is a reasonable starting point for hybrid (it would drop everything for vector/BM25 — tune per retrieval type and app).
...
results = await client.memories.search(
    ...
    # or VectorRetrieval / BM25Retrieval / FetchRetrieval (unranked listing)
)
```
